...
"[project]/OneDrive/Desktop/Minor_Project/FIX-MY-CITY/app/api/voice-agent/route.ts [app-route] (ecmascript)", ((__turbopack_context__) => {
"use strict";
...
const RESPONSE_SCHEMA = {
    type: __TURBOPACK__imported__module__$5b$project$5d2f$OneDrive$2f$Desktop$2f$Minor_Project$2f$FIX$2d$MY$2d$CITY$2f$node_modules$2f40$google$2f$generative$2d$ai$2f$dist$2f$index$2e$mjs__$5b$app$2d$route$5d$__$28$ecmascript$29$__["SchemaType"].OBJECT,
    properties: {
        agent_reply: {
            type: __TURBOPACK__imported__module__$5b$project$5d2f$OneDrive$2f$Desktop$2f$Minor_Project$2f$FIX$2d$MY$2d$CITY$2f$node_modules$2f40$google$2f$generative$2d$ai$2f$dist$2f$index$2e$mjs__$5b$app$2d$route$5d$__$28$ecmascript$29$__["SchemaType"].STRING
        },
        extracted: {
            type: __TURBOPACK__imported__module__$5b$project$5d2f$OneDrive$2f$Desktop$2f$Minor_Project$2f$FIX$2d$MY$2d$CITY$2f$node_modules$2f40$google$2f$generative$2d$ai$2f$dist$2f$index$2e$mjs__$5b$app$2d$route$5d$__$28$ecmascript$29$__["SchemaType"].OBJECT,
            properties: {
        ...
        complete: {
            type: __TURBOPACK__imported__module__$5b$project$5d2f$OneDrive$2f$Desktop$2f$Minor_Project$2f$FIX$2d$MY$2d$CITY$2f$node_modules$2f40$google$2f$generative$2d$ai$2f$dist$2f$index$2e$mjs__$5b$app$2d$route$5d$__$28$ecmascript$29$__["SchemaType"].BOOLEAN
        }
    },
    required: [
        "agent_reply",
...
function normalizeFields(input) {
    return {
        category: isKnownValue(input?.category) ? String(input?.category).trim() : UNKNOWN_VALUE,
        description: isKnownValue(input?.description) ? String(input?.description).trim() : UNKNOWN_VALUE,
        address: isKnownValue(input?.address) ? String(input?.address).trim() : UNKNOWN_VALUE,
        city: isKnownValue(input?.city) ? String(input?.city).trim() : UNKNOWN_VALUE,
        pincode: isKnownValue(input?.pincode) ? String(input?.pincode).trim() : UNKNOWN_VALUE,
        urgency: normalizeUrgency(input?.urgency)
    };
}
function mergeFields(baseFields, incomingFields) {
    const normalizedIncoming = normalizeFields(incomingFields);
...
function applyContextualAnswer(userMessage, currentFields, requestedField) {
...
function areRequiredFieldsComplete(fields) {
    return isKnownValue(fields.category) && isKnownValue(fields.description) && isKnownValue(fields.city) && isKnownValue(fields.address) && /^\d{6}$/.test(fields.pincode.trim());
}
...
function buildFallbackResponse(userMessage, currentFields, history) {
    ...
    const contextFields = applyContextualAnswer(userMessage, currentFields, requestedField);
...
async function POST(req) {
    ...
    const currentExtracted = normalizeFields(body?.currentExtracted);
    if (!userMessage) {
    ...
    try {
        const requestedField = inferRequestedField(history, currentExtracted);
        const contextFields = applyContextualAnswer(userMessage, currentExtracted, requestedField);
        const contextualExtracted = mergeFields(currentExtracted, contextFields);
        const heuristicExtracted = applyHeuristics(userMessage, contextualExtracted);
        if (requestedField && !hasFieldValue(currentExtracted, requestedField) && hasFieldValue(heuristicExtracted, requestedField)) {
...
${userMessage}`;
        ...
        const modelExtracted = normalizeFields(parsed?.extracted);
        const mergedFields = mergeFields(contextualExtracted, modelExtracted);
        const extracted = mergeFields(mergedFields, applyHeuristics(userMessage, mergedFields));
        ...
        return __TURBOPACK__imported__module__$5b$project$5d2f$OneDrive$2f$Desktop$2f$Minor_Project$2f$FIX$2d$MY$2d$CITY$2f$node_modules$2f$next$2f$server$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["NextResponse"].json({
            success: true,
            data: buildFallbackResponse(userMessage, currentExtracted, history)
        });
    }
}
}),
...
//# sourceMappingURL=%5Broot-of-the-server%5D__5b8433bb._.js.map